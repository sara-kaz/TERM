"""
TERM Online RL Fine-Tuning Trainer
====================================
Phase 2: roll out the TERM policy in an environment and optimise using
...
from models.term_model import TERMModel, RMSNorm
...
class ValueHead(nn.Module):
    ...
    Uses RMSNorm (matching TERM's backbone) rather than LayerNorm to keep
...
def rl_update(
    ...
    cfg:        dict,
    device:     str,
    bc_model:   Optional[TERMModel] = None,
) -> dict:
...
def rl_train(cfg: dict):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if mean_success > best_success:
            ...
            }, rl_out_dir / "best_success_term.pt")
        ...
        if epoch % cfg["rl"].get("save_every", 20) == 0:
            torch.save({
                ...
                "model_state":      model.state_dict(),
                "success_rate":     mean_success,
            }, rl_out_dir / f"rl_term_epoch{epoch:04d}.pt")

        # Flush log every 5 epochs so results survive Colab disconnects
        if epoch % 5 == 0:
            with open(rl_out_dir / "rl_term_log.json", "w") as f:
    ...
    print(f"[rl_term] Done. Best return: {best_return:.4f} | "
