"""
TERM Supervised Fine-Tuning (Behavioural Cloning) Trainer
==========================================================
Phase 1: Train the TERM model on expert demonstrations.
...
from models.term_model import TERMModel
    ...
    if ep_path and Path(ep_path).exists():
        if dataset_type == "language_table":
            ...
            print(
                ...
                "disk I/O, may take seterml minutes on first run…",
                flush=True,
            )
...
def load_pretrained_lenient(model: TERMModel, state_dict: dict) -> tuple:
...
def build_model(cfg: dict) -> TERMModel:
    ...
    return TERMModel(
        ...
        use_lang_feedback=term_cfg.get("use_lang_feedback", True),
        use_action_lang_feedback=term_cfg.get("use_action_lang_feedback"),
        use_temporal_history=term_cfg.get("use_temporal_history", True),
        use_reward_gate=term_cfg.get("use_reward_gate", True),
        use_consequence_token=term_cfg.get("use_consequence_token", True),
        action_dim=cfg["model"].get("action_dim", 4),   # 4=MetaWorld, 2=Language-Table, 7=CALVIN
        action_vocab=term_cfg.get("action_vocab"),      # None → use built-in vocabulary
        proprio_dim=cfg["model"].get("proprio_dim", 0),
        chunk_size=cfg["model"].get("chunk_size", 1),
        use_step_conditioning=term_cfg.get("use_step_conditioning", False),
        step_horizon=term_cfg.get(
...
def run_epoch(
    ...
    ce       — cross-entropy on discrete action logits          (always)
               When chunk_size > 1, CE is atermged over all K chunk steps:
    ...
    with ctx:
        ...
        for bi, batch in enumerate(loader):
            if bi == 0:
                print(
                    f"[sft_term] {tag}: first batch ready after {time.perf_counter() - t_batch0:.1f}s "
            ...
            if bi == 0:
                print(
                    f"[sft_term] {tag}: batch0 H2D+forward {time.perf_counter() - t_h2d:.1f}s",
                    flush=True,
                )
            ...
            # K>1: CE atermged over all K chunk steps — K× supervision signal.
            ...
            if is_train:
                ...
                if bi == 0:
                    print(
                        f"[sft_term] {tag}: batch0 backward+optimizer "
                ...
                if device.startswith("cuda"):
                    torch.cuda.synchronize()
                print(
                    f"[sft_term] {tag}: batch0 val total {time.perf_counter() - t_batch0:.1f}s "
                    "(no backward)",
                    flush=True,
                )
            ...
            if (
                ...
                print(
                    f"[sft_term] {tag}: batch {bi + 1}/{n_batches} ({pct:.1f}%) "
    ...
    return {
        ...
        # Alignment cosine diagnostics — atermged over batches (not samples)
...
def train(cfg: dict, resume_from: Optional[str] = None):
    """
    Train TERM with optional warm-restart from a saved checkpoint.
    ...
    resume_from : path to a .pt file produced by this trainer.
                  ...
                  The existing sft_term_log.json is preserved and appended to.
    """
    seed = int(cfg["training"].get("seed", 42))
    ...
    print(f"[sft_term] device = {device} | seed = {seed}", flush=True)
    ...
    align_coef  = float(cfg.get("term", {}).get("alignment_loss_coef", 0.1))
    reg_coef    = float(cfg.get("term", {}).get("regression_loss_coef", 0.5))
        ...
        if finetune:
            ...
            if "scheduler_state" in ckpt:
                ...
                for _ in range(start_epoch):
                    scheduler.step()
            log_path = out_dir / "sft_term_log.json"
            if log_path.exists():
    ...
    for epoch in range(start_epoch + 1, total_epochs + 1):
        ...
        if epoch == start_epoch + 1:
            ...
            print(
                f"[sft_term] Beginning epoch {epoch} — {nbt} train batches; "
        ...
        with open(out_dir / "sft_term_log.json", "w") as f:
        ...
        if improved:
            ...
            torch.save({
                ...
                "cfg":              cfg,
            }, out_dir / "best_sft_term.pt")
            print(f"  ✓ best checkpoint saved ({metric_msg})")
        else:
            patience_counter += 1
            if patience_counter >= patience:
                print(f"\n[sft_term] Early stop at epoch {epoch} "
        ...
        if epoch % cfg["training"].get("save_every", 10) == 0:
            torch.save({
                ...
                "scheduler_state":  scheduler.state_dict(),
                "cfg":              cfg,
            }, out_dir / f"sft_term_epoch{epoch:04d}.pt")

    print(f"\n[sft_term] Done. Best val acc: {best_val_acc:.3f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume_from", default=None,
                        help="Path to a best_sft_term.pt checkpoint to resume from.")
