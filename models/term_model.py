"""
TERM: Vision-Experience-Reasoning-Action Model
===============================================
...
Ablation flags (configs/config.yaml):
  term.use_lang_feedback      → False : Ablation A — no language feedback (base VLA)
  term.use_temporal_history   → False : Ablation B — flat positional history
  term.use_reward_gate        → False : Ablation C — no reward gate on E_act
  term.alignment_loss_coef    → 0     : Ablation D — no contrastive alignment loss
  term.use_consequence_token  → False : Ablation F — action narration only (no E_exp)
"""
...
class LLaMAFusionTransformer(nn.Module):
    ...
    Main TERM fusion uses bidirectional attention (attn_mask=None).
    """
...
class ViLTModalityEmbedding(nn.Module):
    ...
    We extend this to five modality types for TERM:
...
# ═════════════════════════════════════════════════════════════════════════════
# Part 3: Existing TERM Components
# ═════════════════════════════════════════════════════════════════════════════
...
class ActionRewardHistoryEncoder(nn.Module):
    ...
    Args:
        ...
        d_model      : fusion embedding dimension (must match rest of TERM)
    """
...
# ═════════════════════════════════════════════════════════════════════════════
# Part 4: Main TERM Model
# ═════════════════════════════════════════════════════════════════════════════

class TERMModel(nn.Module):
    """
    Vision-Experience-Reasoning-Action (TERM) Model.
    ...
    def forward(
        ...
        # to vision + history (see docs/term_corl.tex §LLaMA Fusion Transformer).
