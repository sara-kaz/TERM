...
def build_task_cache(octo_model, episodes, chunk_size: int = 64):
    """Pre-compute task embeddings for every unique instruction.

    Uses a fixed anchor text as the first element of every chunk so that
    all chunks produce the same token sequence length, guaranteeing a
    single XLA compilation for the backbone during training.
    """
    _ANCHOR = (
...
def main():
    ...
    # Use a cached task (shape consistent with training) for head init.
