...
Usage
-----
  python -m evaluation.evaluate_term \\
...
def build_term_from_cfg(cfg: dict, device: str) -> TERMModel:
    ...
    return TERMModel(
        ...
        action_vocab          = term_cfg.get("action_vocab"),
...
def load_checkpoint(model: TERMModel, checkpoint_path: str, device: str):
...
def run_ablation_table(
    ...
    for name, overrides in ABLATION_PRESETS.items():
        ...
        for k, v in overrides.items():
            ablation_cfg["term"][k] = v

        model = build_term_from_cfg(ablation_cfg, device)
...
def main():
    ...
    parser.add_argument("--seeds",       type=int, default=3,
                        help="Number of random seeds to atermge over")
    ...
    if args.ablation_table:
        ...
        out_path = out_dir / "term_eval_results.json"
        with open(out_path, "w") as f:
            json.dump(results, f, indent=2)
        print(f"\nSaved to {out_path}")
