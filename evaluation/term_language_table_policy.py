"""
TERM policy adapter for the official Language-Table simulation benchmark.

Maps TERM discrete (+ optional regression) outputs to 2-DoF EE deltas in [-0.1, 0.1].
"""
...
# Training data median ‖Δ‖ ≈ 0.039; p90 ≈ 0.084 (see language_table_term stats).
...
class TERMLanguageTablePolicy:
    ...
    def __init__(
        ...
        self.model = build_term_from_cfg(self.cfg, self.device)
        ...
        term_cfg = self.cfg.get("term", {})
        self.use_step_conditioning = bool(term_cfg.get("use_step_conditioning", False))
        ...
        print(
            f"[TERMLanguageTablePolicy] mode={self.action_mode}  "
