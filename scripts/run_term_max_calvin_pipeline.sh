#!/usr/bin/env bash
# TERM-only max CALVIN pipeline — runs A→Z automatically with live progress in pipeline.log
set -euo pipefail
...
PIPE="$ROOT/checkpoints/term_max_calvin"
MAX="$ROOT/checkpoints/calvin_max/seed123"
REG="$ROOT/checkpoints/calvin_term_reg_only/seed123"
...
pick_best_embodied() {
  ...
  for C in "$ckpt_a" "$ckpt_b" "$ckpt_c"; do
    ...
    "$PY" "$ROOT/scripts/run_term_embodied_score.py" \
      --checkpoint "$C" \
      --dataset_path "$CALVIN" \
...
banner "TERM MAX CALVIN — automatic pipeline (TERM-only)"
log "Log file: $LOG"
...
if [[ -f "$MAX/best_sft_term.pt" && "${SKIP_P1:-0}" == "1" ]]; then
  log "SKIP_P1=1 — using existing $MAX/best_sft_term.pt"
else
  P1_RESUME="$WARM"
  P1_EXTRA=""
  if [[ -f "$MAX/sft_term_log.json" ]]; then
    LAST_EPOCH=$("$PY" -c "
import json
d=json.load(open('$MAX/sft_term_log.json'))
print(d[-1]['epoch'] if d else 0)
")
    TARGET=$("$PY" -c "
import yaml
from pathlib import Path
print(yaml.safe_load(Path('$ROOT/configs/calvin_term_max.yaml').read_text())['training']['epochs'])
")
    if [[ "$LAST_EPOCH" -gt 0 && "$LAST_EPOCH" -lt "$TARGET" ]]; then
      EPOCH_PT="$MAX/sft_term_epoch$(printf '%04d' "$LAST_EPOCH").pt"
      if [[ -f "$EPOCH_PT" ]]; then
        P1_RESUME="$EPOCH_PT"
        P1_EXTRA="cfg['training']['finetune'] = False"
        log "Resuming phase 1 from epoch $LAST_EPOCH/$TARGET → $EPOCH_PT"
      elif [[ -f "$MAX/best_sft_term.pt" ]]; then
        P1_RESUME="$MAX/best_sft_term.pt"
        P1_EXTRA="cfg['training']['finetune'] = False"
        log "Resuming phase 1 from best checkpoint (epoch $LAST_EPOCH/$TARGET)"
      fi
    fi
  fi
  run_train "configs/calvin_term_max.yaml" "$P1_RESUME" "$MAX/train.log" "$P1_EXTRA"
fi
[[ -f "$MAX/best_sft_term.pt" ]] || { log "FATAL: missing $MAX/best_sft_term.pt"; exit 1; }
phase_done 1 "Max continuous train"

phase_start 2 "Regression-only finetune (~20 epochs)"
if [[ -f "$REG/best_sft_term.pt" && "${SKIP_P2:-0}" == "1" ]]; then
  log "SKIP_P2=1 — using existing $REG/best_sft_term.pt"
else
  run_train "configs/calvin_term_reg_only.yaml" "$MAX/best_sft_term.pt" "$REG/train.log"
fi
phase_done 2 "Regression-only finetune"

phase_start 3 "Pick best TERM checkpoint (embodied 100-seq)"
if [[ "${SKIP_P3:-0}" == "1" && -f "$PIPE/best_embodied.pt" ]]; then
  log "SKIP_P3=1 — using existing $PIPE/best_embodied.pt"
else
  pick_best_embodied "$MAX/best_sft_term.pt" "$REG/best_sft_term.pt" "$WARM" 1 "$PIPE/embodied_pick_p3.json"
fi
...
pick_best_embodied "${D2}/best_sft_term.pt" "${D1}/best_sft_term.pt" "${REG}/best_sft_term.pt" 1 "$PIPE/embodied_pick_final.json"
...
for HOLD in 1 5 10; do
  ...
  "$PY" "$ROOT/scripts/run_calvin_rollout_eval.py" \
    ...
    --lang_goal task_key \
    --eval_log_dir "$OUT"
  SR=$("$PY" -c "import json; print(json.load(open('$OUT/term_calvin_summary.json'))['chain_success_rate']['1'])")
...
phase_start 10 "Official 1000-chain TERM eval"
FINAL_OUT="$PIPE/calvin_rollout_1000_term"
setup_eval
...
if s.exists():
    ...
    print()
    print('=' * 60)
    print('  TERM CALVIN FINAL RESULTS')
    print('=' * 60)
...
log "Best TERM ckpt: $PIPE/best_embodied.pt"
