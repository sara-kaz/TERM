#!/usr/bin/env bash
# Boost TERM embodied CALVIN: factored gripper + 10× DAgger data + R3 finetune + eval.
set -euo pipefail
...
EVAL_GPU="${EVAL_GPU:-2}"
PIPE="$ROOT/checkpoints/term_boost_calvin"
LOCK="$PIPE/boost.lock"
CKPT="${CKPT:-$ROOT/checkpoints/calvin_max_dagger_r2/seed123/best_sft_term.pt}"
...
import json
from pathlib import Path
s=json.load(open('$PIPE/final_1000/term_calvin_summary.json'))
report={
  'method':'TERM_boost_r3',
  'checkpoint': '$BEST',
  'action_mode':'continuous',
  'action_magnitude':1.0,
  'action_hold_steps':1,
