#!/usr/bin/env python3
"""
Pre-flight gate before expensive TERM retrain / ablation runs.
...
Usage:
  python scripts/preflight_term.py
  python scripts/preflight_term.py --calvin_path ~/calvin_task_D/task_D_D
...
def check_configs():
    ...
    if cal.get("term", {}).get("use_action_lang_feedback") is not False:
...
def check_ablation_architectures():
    ...
    sigs = {}
    for name, term_ov in variants.items():
        c = copy.deepcopy(base)
        c["term"].update(term_ov)
        m = build_term_from_cfg(c, "cpu")
        m.cpu()
        sig = (
    ...
    if sigs["no_exp"] != sigs["full_term"]:
        ok("no_exp differs from full_term")
    else:
        fail("no_exp identical to full_term")

    if sigs["bc_baseline"] != sigs["full_term"]:
        ok("bc_baseline differs from full_term")
    else:
        fail("bc_baseline identical to full_term")
...
def check_ce_supervision():
    ...
    from training.sft_trainer_term import build_model
...
def check_bidirectional_fusion():
    ...
    import inspect
    from models import term_model as vm
    src = inspect.getsource(vm.TERMModel.forward)
...
def main():
    p = argparse.ArgumentParser(description="TERM pre-flight checks")
    ...
    print("=" * 60)
    print("  TERM PRE-FLIGHT (CALVIN + Language-Table)")
    print("=" * 60)
