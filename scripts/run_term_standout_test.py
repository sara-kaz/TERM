#!/usr/bin/env python3
"""
TERM stand-out test — closed-loop CALVIN comparison on the same eval protocol.

Runs rollout eval for conditions designed to show *where TERM differs from BC*:

  1. full_term_history_on   — Full TERM checkpoint, temporal history active (default)
  2. full_term_history_off  — Same checkpoint, --reset_history_each_step (ablates Stream 4 at eval)
...
If TERM is effective, expect:
  - (1) > (2)  — history helps closed-loop control on the *same* weights
  - (1) >= (3) — full TERM beats or matches BC in sim (not just offline val acc)

Usage
-----
  cd ~/work/RLConditionedVLA
  CUDA_VISIBLE_DEVICES=2 python scripts/run_term_standout_test.py \\
      --dataset_path ~/calvin_task_D/task_D_D \\
      --num_sequences 200

Smoke (10 seq, ~5 min):
  python scripts/run_term_standout_test.py --num_sequences 10 --smoke
"""
...
DEFAULT_OUT = ROOT / "checkpoints" / "term_standout_test"
PY = os.environ.get(
    "VENV_PYTHON",
...
CONDITIONS = [
    {
        "id": "full_term_history_on",
        "label": "Full TERM (history ON)",
        "checkpoint": _ckpt("full_term", "seed123", "best_sft_term.pt"),
        "reset_history": False,
    },
    {
        "id": "full_term_history_off",
        "label": "Full TERM (history OFF, same ckpt)",
        "checkpoint": _ckpt("full_term", "seed123", "best_sft_term.pt"),
        "reset_history": True,
    },
    {
        ...
        "checkpoint": _ckpt("bc_baseline", "seed123", "best_sft_term.pt"),
        "reset_history": False,
    },
    {
        "id": "no_history_tf",
        "label": "No history TF (trained ablation)",
        "checkpoint": _ckpt("no_history_tf", "seed123", "best_sft_term.pt"),
        "reset_history": False,
    },
]
...
def run_one(
    ...
    *,
    ...
    summary_path = edir / "term_calvin_summary.json"
...
def print_table(results: list[dict]) -> None:
    print("\n" + "=" * 72)
    print("  TERM STAND-OUT TEST — CALVIN chain success")
    print("=" * 72)
    ...
    # Deltas vs full_term_history_on
    base = next((r for r in results if r.get("id") == "full_term_history_on" and not r.get("error")), None)
    if base:
        b1 = float(base.get("chain_success_rate", {}).get("1", 0))
        print(f"\n  Δ 1-task success vs Full TERM (history ON):")
        for r in results:
            if r.get("error") or r.get("id") == "full_term_history_on":
                continue
    ...
    print("    • Full TERM > BC            → language+history design beats plain SFT")
    print("    • Full TERM > No-history TF → trained history stream matters in sim")
    print("=" * 72 + "\n")


def main():
    p = argparse.ArgumentParser(description="TERM stand-out CALVIN comparison test")
