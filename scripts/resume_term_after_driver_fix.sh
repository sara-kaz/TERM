#!/usr/bin/env bash
# Resume TERM max pipeline after driver fix / reboot.
set -euo pipefail
...
if [[ "$LAST_EPOCH" -ge "$TARGET_EPOCHS" && -f "$MAX/best_sft_term.pt" ]]; then
  export SKIP_P1=1
...
if [[ -f "$REG/best_sft_term.pt" ]]; then
  export SKIP_P2=1
...
cd "$ROOT"
bash scripts/start_term_max_calvin.sh
