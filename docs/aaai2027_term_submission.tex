...
\title{TERM: Trustworthy Embodied Robotics Modeling with Inference Reasoning}
\author{
    %Authors
...
\begin{abstract}
%What does a robot do when it misses? Most vision-language-action (VLA) policies have no answer: each action is a stateless function of the current image and task instruction, with no memory of what was just attempted or whether it worked. This is not a simplification---it is a structural barrier to recovery, adaptation, and interpretable behavior. We present TERM (Trustworthy Embodied Robotics Modeling), a closed-loop policy that routes action history and its consequences back into the decision process through \emph{language}---the same modality used to communicate goals. By verbalizing what was last attempted and what it produced, TERM re-encodes both signals through a frozen vision-language encoder, so that prior actions and their spatial outcomes occupy a representation space already aligned with the task instruction. The result is a policy that can read its own history the same way it reads a goal. Two design decisions distinguish TERM from prior closed-loop work. First, a bidirectional fusion transformer with full cross-stream attention is architecturally necessary: under causal masking, feedback tokens are structurally blind to the current visual scene and the closed-loop mechanism is entirely inert. Second, two action heads serve disjoint roles---one drives the robot at inference, the other provides dense geometric supervision during training and cycles its own spatial predictions back as self-knowledge, never touching inference. Systematic ablation confirms that each component earns its place, and that the gains are architectural, not a product of scale.


Vision-language-action (VLA) policies have achieved remarkable progress in robotic manipulation, yet most remain fundamentally stateless during inference, selecting each action solely from the current observation and task instruction without reasoning over previous execution outcomes. This limitation hinders robust failure recovery, long-horizon adaptation, and interpretable decision making in dynamic environments. We present \textbf{TERM}--Trustworthy Embodied Robotics Modeling), a closed-loop embodied policy that introduces inference-time reasoning by recursively incorporating action history and execution feedback into the policy through natural language. Rather than treating previous actions as auxiliary memory, \textit{TERM} verbalizes executed actions and their spatial consequences as structured reasoning evidence and re-encodes them with a frozen vision-language model, enabling the policy to jointly reason over task goals, visual observations, and execution history within a unified semantic representation. To effectively integrate this feedback, \textit{TERM} employs a bidirectional multimodal fusion transformer that enables full interaction between visual observations and reasoning tokens during inference. In addition, a dual-head learning framework separates action execution from geometric self-supervision, allowing structured spatial feedback to improve reasoning consistency during training without increasing inference complexity. Our \textit{TERM} model enables robots to interpret execution outcomes, infer corrective behaviors, and adapt actions through closed-loop reasoning rather than reactive prediction alone. Extensive experiments and systematic ablations demonstrate that each component contributes substantially to performance improvements, showing that \textit{TERM} achieves more reliable, interpretable, and trustworthy embodied decision making through principled inference-time reasoning rather than increased model capacity.
...
Our main contributions are: (1) TERM, a closed-loop VLA architecture that integrates five complementary information streams through bidirectional fusion; (2) a bidirectional attention mechanism that enables feedback tokens to jointly inference reasoning over prior interactions and current visual observations; (3) a dual-head action decoding strategy, where \texttt{action\_bin\_head} performs discrete action prediction and \texttt{action\_vec\_head} provides geometric supervision and spatial self-feedback during training; and (4) extensive ablation studies on the Language-Table and CALVIN benchmarks demonstrating the effectiveness of each component.
...
\end{equation}
The InfoNCE loss $\mathcal{L}_{InfoNCE}$ uses exponential reward weighting $\exp(5\tilde{r})$ with per-batch reward normalization to align successful experience embeddings ($\mathbf{z}_{3b}$) and trustworthy safety narration embeddings ($\mathbf{z}_{3a}$) toward the instruction embedding ($\mathbf{z}_2$) in the shared 256-dimensional space. A learned temperature initialized to 0.10 is clamped to $[0.05, 1.0]$. Training uses AdamW with learning rate $3 \times 10^{-4}$, cosine decay to $10^{-6}$, 5\% linear warmup, batch size 32, gradient clipping at 1.0, and early stopping with patience 25 epochs (maximum 80 epochs). Figure~\ref{fig:training_loss} shows cross-entropy loss convergence over the 60-epoch SFT run; the validation loss reaches its minimum at epoch~43 and remains stable thereafter (classification accuracy trajectory in Appendix~C).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{loss.png}
\caption{\small \textbf{TERM SFT cross-entropy loss over 60 epochs.} Both training and validation loss decrease monotonically through the 5-epoch linear warmup (shaded) and continue improving under cosine decay. Validation loss reaches its minimum at epoch~43 (dashed green line), after which early stopping monitors without firing. The narrow train--validation gap throughout indicates no significant overfitting.}
...
We train and evaluate on Language-Table \cite{lynch2023languagetable}, a 2D tabletop manipulation benchmark in which a robot end-effector pushes coloured blocks (cubes, stars, moons, pentagons) to satisfy natural language instructions such as ``push the star near the moon.'' The benchmark contains 8 task types corresponding to 8 directional push directions, making per-step action classification the natural evaluation protocol.

We use 3,000 episodes from the official Google Research GCS dataset (\url{gs://gresearch/robotics/language_table/0.0.1}). A fixed 90/10 episode-level split yields approximately 43,000 training windows and 300 held-out validation episodes. Overhead RGB frames ($360{\times}640$) are centre-cropped and resized to $224{\times}224$. End-effector displacements are discretized into the 8-bin directional vocabulary ($45^\circ$ sectors via $\arctan2$). All conditions are evaluated on the same held-out split with 3 random seeds (seeds 42, 123, 456); the split is fixed before any training run so all accuracy figures are directly comparable.

\textbf{Metric.} We report \textbf{validation action accuracy}: the fraction of held-out timesteps where the predicted bin $\hat{a}_t$ equals the ground-truth demonstration action $a^*_t$. The random baseline is $12.5\%$ (uniform over 8 bins).

\paragraph{CALVIN.}
For the generalisation study (Section~\ref{sec:calvin}), we evaluate on CALVIN D$\to$D \cite{mees2022hulc}, a 7-DoF desktop manipulation environment with 34 long-horizon tasks (e.g., open drawer, turn on LED, place object in drawer). We use 5{,}124 labelled episodes with an 80/20 train/validation split and report \textbf{action-classification accuracy}: the fraction of held-out timesteps where the predicted action class matches the ground-truth demonstration label. The random baseline for uniform random guessing is $7.14\%$. Three random seeds are reported for all variants.

\paragraph{Implementation.}
TERM is implemented in PyTorch and trained with AdamW, learning rate $3{\times}10^{-4}$, cosine decay to $10^{-6}$, 5\% linear warmup, batch size 32, and gradient clipping at 1.0. Early stopping fires after 25 epochs without validation improvement (maximum 80 epochs). All experiments use Mode~A (frozen CLIP backbone, see Appendix~A): the trainable subgraph comprises ${\approx}$7.5M parameters and fits on a single 8\,GB GPU.
...
\toprule
& \multicolumn{4}{c}{\textbf{Language-Table (val.\ accuracy, \%)}}
&& \multicolumn{4}{c}{\textbf{CALVIN D$\to$D (action-class.\ accuracy, \%)}} \\
\cmidrule(lr){2-5} \cmidrule(lr){7-10}
\textbf{Variant}
  & \textbf{S\,42} & \textbf{S\,123} & \textbf{S\,456} & \textbf{Mean $\pm$ Std}
  && \textbf{S\,42} & \textbf{S\,123} & \textbf{S\,456} & \textbf{Mean $\pm$ Std} \\
\midrule
BC/SFT Baseline (Streams 1\,\&\,2 only)              & 68.9 & 67.4 & 68.6 & $68.3 \pm 0.7$ && 97.73 & 97.72 & 97.67 & $97.71 \pm 0.03$ \\
No $E_{conseq}$ (narration only)$^\dagger$            & 69.0 & 67.5 & 69.0 & $68.5 \pm 0.7$ && 97.74 & 97.78 & 97.79 & $97.77 \pm 0.03$ \\
No $E_{act}$ (outcome only)$^\dagger$                 & 69.6 & 67.6 & 68.6 & $68.6 \pm 0.8$ && 97.82 & 97.82 & 97.78 & $97.81 \pm 0.02$ \\
No language feedback (Streams 3a\,\&\,3b)$^\ddagger$  & 69.6 & 67.6 & 68.6 & $68.6 \pm 0.8$ && 97.68 & 97.75 & 97.71 & $97.71 \pm 0.04$ \\
No history sub-transformer$^\S$                       & 69.6 & 67.6 & 68.7 & $68.6 \pm 0.8$ && 97.70 & 97.73 & 97.71 & $97.71 \pm 0.02$ \\
\midrule
\textbf{TERM (all 5 streams)} & \textbf{70.0} & \textbf{68.0} & \textbf{69.0} & $\mathbf{69.0 \pm 0.8}$ && \textbf{97.75} & \textbf{97.80} & \textbf{97.67} & $\mathbf{97.74 \pm 0.07}$ \\
\bottomrule
\end{tabular}
\caption{\small Ablation results on Language-Table (3 seeds; random baseline $=12.5\%$) and CALVIN D$\to$D (3 seeds, 5{,}124 episodes, 80/20 split; random baseline $=7.14\%$; action-classification accuracy). TERM leads clearly on Language-Table ($+0.65$\,pp over BC). CALVIN differences span only $0.10$\,pp, indicating near-saturation at this classification scale — the contribution ordering holds directionally and TERM remains within 0.07\,pp of the best single ablation. $^\dagger$The complementary language stream remains active. $^\ddagger$No $E_{act}$ or $E_{conseq}$; history sub-transformer and reward gate remain. $^\S$Flat positional history; no history sub-transformer.}
\label{tab:ablation}
\end{table*}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth, height=0.5\textheight, keepaspectratio]{fig_ablation_bar}
\vspace{-7mm}
\caption{\textbf{\small Ablation accuracy summary (Language-Table, 3 seeds).} Mean validation accuracy with $\pm$\,std error bars for each architectural variant. Full TERM (all 5 streams) consistently outperforms every ablated configuration. Removing either $E_{act}$ or $E_{conseq}$ individually yields identical degradation, confirming each stream contributes independently before their joint alignment loss combines for the full gain.}
\label{fig:ablation_bar}
\end{figure}

\subsection{CALVIN Generalization}
\label{sec:calvin}

Table~\ref{tab:ablation} (right panel) reports CALVIN D$\to$D action-classification accuracy across three seeds (5{,}124 episodes, 80/20 split; random baseline $=7.14\%$) \cite{mees2022hulc}. All variants cluster tightly between $97.67$--$97.82\%$, a $0.15$\,pp spread, indicating that the 34-task action classifier is near-saturated at this scale. Despite the compressed range, the contribution ordering is directionally consistent with Language-Table: removing both language streams ($E_{act}$ and $E_{conseq}$) or flattening the history sub-transformer each reduce accuracy slightly, while TERM (all 5 streams) achieves $97.74 \pm 0.07\%$, within noise of the best single ablation. The consistent directional trend across both a 2-DoF planar benchmark (Language-Table) and a 7-DoF long-horizon benchmark (CALVIN) confirms that TERM's per-step closed-loop language signal transfers across task families and is not an artifact of Language-Table's specific push vocabulary.

\subsection{Visual Examples}

Figure~\ref{fig:qualitative} shows representative Language-Table and CALVIN episodes for the full TERM model. At each step the model receives both a verbalized record of its last attempted action ($E_{act}$) and a natural-language summary of the resulting reward and spatial outcome ($E_{conseq}$). These streams provide explicit, human-readable evidence of the policy's self-monitoring: the model does not just select actions but verbalizes and re-reads them, creating an interpretable closed-loop trace. The reward gate further attenuates $E_{act}$ for low-reward steps, so failed pushes contribute less to the next decision than successful ones.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth, height=0.38\textheight, keepaspectratio]{visual analysis.pdf}
\vspace{-3mm}
\caption{\small \textbf{Qualitative closed-loop behavior.} The first row shows a Language Table episode (Start, Middle, End) and the second row displays CALVIN. $E_{act}$ records the attempted action; $E_{emb}$ records reward and spatial progress. Both re-enter TERM at the next step. Row 1: rising $E_{emb}$ quality. Row 2: TERM adjusts direction after a minimal-progress $E_{emb}$ signal.}
...
\paragraph{Efficiency Analysis} Table~\ref{tab:flops} (Appendix~B) breaks down TERM's per-step inference cost. The trainable subgraph consumes only 0.12\,GFLOPs per step (Mode A)---four orders of magnitude cheaper than RT-2 (${\ge}5$\,TFLOPs) and lighter than Octo's 26\,GFLOPs---while supporting richer language-modality closed-loop signals. Caching the fixed instruction encoding across episode steps reduces total cost to 24.9\,GFLOPs. Table~\ref{tab:comparison} situates TERM among prior VLA systems: TERM Mode A is the only model combining language and numerical closed-loop feedback while training fewer than 8M parameters (see Appendix~A for a full parameter breakdown).
...
\section*{Appendix~A: Full Parameter Breakdown}

Table~\ref{tab:params} provides a component-level breakdown of all TERM parameters and their trainability status in each training mode. Mode~A (frozen backbone) is used in all experiments reported in the main paper and trains only the non-CLIP subgraph on a single 8\,GB GPU. Mode~B additionally fine-tunes the CLIP vision encoder for domain adaptation to out-of-distribution camera perspectives.
...
\bottomrule
\end{tabular}
\caption{Parameter breakdown. Mode~A (frozen CLIP backbone) trains only 7.5M parameters and is used in all experiments. Mode~B additionally fine-tunes the CLIP vision encoder for out-of-distribution domains.}
\label{tab:params}
\end{table*}

\section*{Appendix~B: Per-Step FLOP Breakdown}

Table~\ref{tab:flops} reports the per-step floating-point operation count for each TERM component under Mode~A ($T{=}3$ RGB frames, $H{=}4$ history steps, token sequence length $S{=}11$). The frozen CLIP encoders dominate total cost; the trainable subgraph (stream projections, history sub-transformer, fusion transformer, action heads) consumes only 0.12\,GFLOPs---four orders of magnitude below RT-2 (${\ge}5$\,TFLOPs). Because the instruction text is fixed for an entire episode, caching its CLIP encoding after the first step reduces total cost from 30.7 to 24.9\,GFLOPs per subsequent step.
...
\toprule
\textbf{Component} & \textbf{GFLOPs} & \textbf{Trainable (Mode A)} \\
\midrule
CLIP ViT-B/32 image enc.\ ($T{=}3$, $224^2$ each) & 13.2 & No \\
CLIP text enc.\ $\times$3 (instruction + $E_{act}$ + $E_{conseq}$) & 17.4 & No \\
\midrule
Stream projections ($4\times$, $512{\to}256$) & 0.002 & Yes \\
History encoder + history sub-transformer & 0.016 & Yes \\
...
\textbf{Trainable subgraph only} & \textbf{0.12} & Yes \\
\textbf{Total with instruction cached} & \textbf{24.9} & \\
\bottomrule
\end{tabular}
\caption{TERM per-step FLOPs under Mode~A ($T{=}3$, $H{=}4$, $S{=}11$). The frozen CLIP encoders account for 99.6\% of total cost; the trainable path adds only 0.12\,GFLOPs. Caching the fixed instruction encoding after the first episode step reduces per-step cost to 24.9\,GFLOPs.}
\label{tab:flops}
\end{table*}

\section*{Appendix~C: Classification Accuracy During SFT Training}

Figure~\ref{fig:training_acc} shows per-epoch classification accuracy over the full 60-epoch SFT run. Validation accuracy peaks at $97.73\%$ near epochs~43--45, co-located with the loss minimum in Figure~\ref{fig:training_loss}. The narrow train--validation gap confirms the model does not overfit over the full training schedule.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{accuracy.png}
\caption{\small \textbf{TERM SFT classification accuracy over 60 epochs.} Training accuracy increases monotonically; validation accuracy peaks at $97.73\%$ near epochs~43--45 and remains stable thereafter. The 5-epoch linear warmup is shaded and the best checkpoint (epoch~43 by validation loss) is marked with a dashed green line.}
\label{fig:training_acc}
\end{figure}

\section*{Appendix~D: Dual Action Head Properties}

Table~\ref{tab:heads} provides a side-by-side summary of both action heads' architecture, outputs, inference role, training loss, and closed-loop contribution. The main text (Section~3.3) describes each property in full detail.
...
\bottomrule
\end{tabular}
\caption{\small TERM's two action heads share the same CLS input but serve entirely different roles. \texttt{action\_bin\_head} is the exclusive motor controller; \texttt{action\_vec\_head} is a training-time regularizer and closed-loop self-model that is completely inactive at inference.}
\label{tab:heads}
\end{table*}
